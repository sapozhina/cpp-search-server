...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        docs_to_status_[document_id]={ComputeAverageRating(ratings), status};
    }

    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status) const {            
        const Query query = ParseQuery(raw_query);
        auto matched_documents = FindAllDocuments(query);
     for (const  auto&  [id, relevance, rating]: matched_documents) {
            if  (!(docs_to_status_.at(id).status == status)) {
                matched_documents.erase(matched_documents.begin()+id);
            }
        } 
...
private:
   ...
   map<int,  Rating_status>  docs_to_status_;
